...
class policy(nn.Module):
    ...
    def act(self, state): 
        with torch.no_grad():
            ...
            q_value = self.forward(state.reshape((1,1,7,21)))
...
def main():
    # Enviroment
    env = UnityEnvironment(file_name="EXE\Client\CRML", seed=1, side_channels=[], worker_id=int(1),no_graphics = True) ## work_id need to be int 
    env.reset()
    ...
    try:
        ...
        for epi in range(N_EPOCHS):
            ...
            while not(done):
                # Get State
                state, reward, done = get_state(env, behavior)
                if not done:
                    # Set Action
                    action = pi.act(state)
        ...
        plt.xlabel('Step')
        plt.savefig(f'.\\{FOLDER}\\1-channel-state\\{time_stamp}\score.png')
        plt.close()
